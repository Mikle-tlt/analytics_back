...
public class OfflineSalesByCategoryService {
    ...
    public Map<String, Object> getSalesByCategory(Long categoryId, int year) {
        ...
        for (Products p : category.getProducts()) {
            ...
            for (int i = 0; i < quantities.length; i++) {
                int finalI = i;
                quantities[i] += product.getOfflinePointProductsList().stream().reduce(0,
                        (op, offlineProduct) -> op + offlineProduct.getOfflineDetails().stream().reduce(0, (d, detail) -> {
                    String date;
                    if (finalI < 9) date = year + "-0" + (finalI + 1);
                    else date = year + "-" + (finalI + 1);
                    if (detail.getOfflineBuy().getDate().startsWith(date)) return d + detail.getQuantity();
                    return d;
    ...
    private String analyzeSeasonalPatterns(int[] monthlySales) {
        List<Integer> maxIndices = findIndicesOfMaxValues(monthlySales);
        List<Integer> minIndices = findIndicesOfMinValues(monthlySales);
    ...
    private List<Integer> findIndicesOfMaxValues(int[] array) {
    ...
    private List<Integer> findIndicesOfMinValues(int[] array) {
        List<Integer> indices = new ArrayList<>();
    ...
    private String forecastFutureSales(int[] monthlySales) {
        ...
        for (int i = 0; i < forecastedSales.length; i++) {
            if(i + 1 != forecastedSales.length) {
                forecastSales.append(getMonthName(i + 1)).append(": ").append(formatNumber(forecastedSales[i])).append("; ");
            }
            else{
                forecastSales.append(getMonthName(i + 1)).append(": ").append(formatNumber(forecastedSales[i])).append(". ");
            }
        }
        return forecastSales.toString();
    }
